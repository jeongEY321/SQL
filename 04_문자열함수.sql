...
SELECT 'abcdef', LENGTH('abcdefS'), INSTR('abcedf', 'a')
FROM dual;
...
SELECT 'abcdef' AS ex, SUBSTR('abcdef', 1, 4), CONCAT('abc', 'edf')
FROM dual;
...
SELECT REPLACE('My dream is a president', ' ', '') -- 공백을 지우겠다
FROM dual;

SELECT REPLACE(REPLACE('My dream is a president', 'president', 'programmer'), ' ', '')
FROM dual; -- 두번 REPLACE를 통해 글자도 바꾸고 공백도 지우기.
...
조건 2) 입사일자 컬럼은 xx/xx/xx/로 저장되어 있습니다. xxxxxx형태로 변경해서 출력합니다.
*/

SELECT first_name || ' ' || last_name, REPLACE (hire_date, '/', '')
FROM employees
