...
-- 표시가 안되어있어도 시간데이터도 가지고 있다.
SELECT TO_CHAR(sysdate) FROM dual;
-- 문자형태로 바꿔 표현 DAY는 요일표시 DY는 요일을 약어로 표시 PM AM은 오전 오후를 표시해준다.
SELECT TO_CHAR(sysdate, 'YYYY-MM-DD DY AM HH:MI:SS') FROM dual;
--년도 앞 2자리 지우고 시간은 24시간제로 표시.
...
SELECT TO_CHAR(20000, '99999') FROM dual; --9라는건 숫자가아닌 자릿수이다.
-- 주어진 자릿수에 숫자를 모두 표기할 수 없어서 모두 #으로 표기됩니다.
SELECT TO_CHAR(20000, '9999') FROM dual; 
SELECT TO_CHAR(20000.21, '99,999.9') FROM dual; 

SELECT TO_CHAR(salary, 'L99,999') AS salary --L을 넣으면 그 나라의 단위에 맞춰서 변한다.
FROM employees;

-- 문자를 숫자로 TO_NUMBER(값, 형식)
SELECT '2000' + 2000 FROM dual; --자동 변환 (문자 -> 숫자)
...
-- 간단문제1) xxxx년 xx월 xx일 문자열 형식으로 변환해 보세요.
...
SELECT
    ...
    (CASE
        ...
        WHEN salary BETWEEN 2000 AND 3999 THEN 'B'
        ELSE 'A'
    END)AS grade
FROM employees
ORDER BY salary DESC;