...
SELECT rn, first_name || ' ' || last_name AS name
FROM
    (
...
SELECT rn, employee_id, first_name || ' ' || last_name AS name, phone_number, hire_date
FROM
    (
    SELECT ROWNUM AS rn, tbl.*
    FROM
...
SELECT loc.location_id, loc.street_address, loc.city, loc.country_id, co.country_name
FROM locations loc LEFT JOIN countries co
...
select l.location_id, l.street_address, l.city, l.country_id, 
    ...
    from countries co
    where l.country_id = co.country_id) as ctn
from locations l
order by ctn asc;
...
SELECT
    d.department_id, d.department_name, d.manager_id, tbl.total
FROM departments d
JOIN 
    (
...
SELECT
    d.*, loc.street_address, loc.postal_code, NVL(tbl.result, 0) AS 부서별평균급여
