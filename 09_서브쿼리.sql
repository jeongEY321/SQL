...
SELECT * FROM employees WHERE job_id = (SELECT job_id FROM employees WHERE job_id = 'IT_PROG');

-- 다중행 연산자
-- IN: 목록의 어떤 값과 같은 지 확인합니다.
SELECT * FROM employees WHERE job_id IN (SELECT job_id FROM employees WHERE job_id = 'IT_PROG');
